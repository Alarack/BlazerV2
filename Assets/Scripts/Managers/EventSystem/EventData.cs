...
public class EventData {
    // TODO: Spam errors if using a field that doesn't exist and default value is being returned

    #region Fields and Properties

    /// <summary> The event (type) that originated (sent) this data package </summary>
    public GameEvent eventType;
    // TODO: Slight spaghetti; we shouldn't have listeners who don't know what they are listening to
    // In other words, if the listener doesn't know about GameEvent type, they probably shouldn't be listening
    // Systems too broad to know what event arrives shouldn't filter them by type,
    // because that defeats the purpose of event system where one knows exactly what to expect --Rudy
    ...
    ///// <summary> The extra/additional keyed Unity Actions delegate data values, if any, sent with this event </summary>
    //private IDictionary<string, GarrisonDataWrapper> _garrisonData;
    ...
    // TODO: ?
    //public EventData(params object[] values)
    //{
    //    init ();
    //    foreach (object value in values)
    //    {
    //        if (value is int) _defaultInt = (int)value;
    //        if (value is string) _defaultString = (string)value;
    //        if (value is float) _defaultFloat = (float)value;
    //        if (value is bool) _defaultBool = (bool)value;
    //        ...
    //    }
    //}

    public EventData() {
        // Prepare for debug-time future verification checks
        __InitializeChecks();
    }
    ...
    //public void AddGarrisonData(string key, GarrisonDataWrapper value) {
    //    // Debug-time verification that we can write the data at this time with the given key
    //    __CheckWritable(key);

    //    if (_garrisonData == null)
    //        _garrisonData = new Dictionary<string, GarrisonDataWrapper>();

    //    _garrisonData.Add(key, value);
    //}
    ...
    //public GarrisonDataWrapper GetGarrisonData(string key) {
    //    // Debug-time verification that we can read the data at this time with the given key
    //    __CheckReadable(key);

    //    GarrisonDataWrapper garrisonData;
    //    if (_garrisonData == null || !_garrisonData.TryGetValue(key, out garrisonData)) {
    //        garrisonData = new GarrisonDataWrapper();
    //    }
